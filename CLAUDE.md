...
1. 运行项目初始化脚本（如 `operators/{operator_name}/` 已存在则跳过）：
   ```bash
   bash /Users/jonas/.codeartsdoer/skills/cicd-pr-analyzer/external/cannbot-skills/plugins-official/ops-direct-invoke/workflows/scripts/init_operator_project.sh {operator_name}
   ```
2. 加载 `/ascendc-env-check` skill，按 skill 指引完成 CANN 环境检查与 NPU 设备检查。
3. 读取模板 `/Users/jonas/.codeartsdoer/skills/cicd-pr-analyzer/external/cannbot-skills/plugins-official/ops-direct-invoke/workflows/templates/environment-template.md`，按其中的「字段语义」表把上一步采集到的信息填入 `operators/{operator_name}/docs/environment.md`。任一 ❌ 错误项（不含 ⚠ 警告） → 状态行写 `❌ 失败`；否则写 `✅ 通过`。
...
**触发条件**：Step 1 通过
**调用模板**：[Step 2](/Users/jonas/.codeartsdoer/skills/cicd-pr-analyzer/external/cannbot-skills/plugins-official/ops-direct-invoke/workflows/task-prompts.md#step-2设计) — 读取此链接的完整内容作为 prompt
...
**调用模板**：[Step 2.5](/Users/jonas/.codeartsdoer/skills/cicd-pr-analyzer/external/cannbot-skills/plugins-official/ops-direct-invoke/workflows/task-prompts.md#step-25设计串讲) — 读取此链接的完整内容作为 prompt
...
**调用模板**：[Step 3](/Users/jonas/.codeartsdoer/skills/cicd-pr-analyzer/external/cannbot-skills/plugins-official/ops-direct-invoke/workflows/task-prompts.md#step-3开发) — 读取此链接的完整内容作为 prompt
**完成判定**：Developer 返回开发概要，代码文件存在于 `operators/{operator_name}/`

#### Step 4：审查

**触发条件**：Developer 完成开发
**调用模板**：[Step 4](/Users/jonas/.codeartsdoer/skills/cicd-pr-analyzer/external/cannbot-skills/plugins-official/ops-direct-invoke/workflows/task-prompts.md#step-4审查) — 读取此链接的完整内容作为 prompt
...
**调用模板**：[Step 5](/Users/jonas/.codeartsdoer/skills/cicd-pr-analyzer/external/cannbot-skills/plugins-official/ops-direct-invoke/workflows/task-prompts.md#step-5修复循环) — 读取此链接的完整内容作为 prompt
...
**调用模板**：[Step 6](/Users/jonas/.codeartsdoer/skills/cicd-pr-analyzer/external/cannbot-skills/plugins-official/ops-direct-invoke/workflows/task-prompts.md#step-6性能验收) — 读取此链接的完整内容作为 prompt
...
| # | 规则 |
|---|------|
| S1 | 调用任何 Subagent 前，**必须先读取** `/Users/jonas/.codeartsdoer/skills/cicd-pr-analyzer/external/cannbot-skills/plugins-official/ops-direct-invoke/workflows/task-prompts.md` 中对应 Step 的完整 prompt 模板 |
...
| 资源类型 | 路径 | 说明 |
|---------|------|------|
| API 文档 | `/Users/jonas/.codeartsdoer/skills/cicd-pr-analyzer/external/cannbot-skills/plugins-official/ops-direct-invoke/asc-devkit/docs/api/context/` | 仲裁 API 争议时查阅 |
| 官方示例 | `/Users/jonas/.codeartsdoer/skills/cicd-pr-analyzer/external/cannbot-skills/plugins-official/ops-direct-invoke/asc-devkit/examples/` | 仲裁开发争议时参考 |
