...
class ProcessInfo:
    ...
    def __init__(
        self, pid: int, session_id: str, step_id: str = "", work_dir: str = ""
    ):
        self.pid = pid
...
class ClaudeCodeDriver:
    ...
    async def run_step(
        ...
        persist_proc_on_consumer_exit: 当消费者（async generator 的迭代方）提前断开
        （GeneratorExit，如前端 SSE 断开）时，若为 True 则不杀 Claude 进程，让它继续
        在后台跑完；若为 False（默认，旧行为）则 kill 进程。后台执行 Task 应传 True，
        使仿真执行与前端 SSE 连接解耦。
        ...
        try:
            ...
            logger.info(
                f"[claude] 进程启动 PID={proc.pid} session={session_id} step={step_id}"
            )
                    ...
                    if event_type == "result":
                        ...
                        if model_usage:
                            for model_name, mu in model_usage.items():
                                tokens["input"] = tokens.get("input", 0) + mu.get(
                                    "inputTokens", 0
                                )
                                tokens["output"] = tokens.get("output", 0) + mu.get(
                                    "outputTokens", 0
                                )
                        elif usage:
            ...
            yield {
                "type": "error",
                "content": "Claude Code CLI 未找到，请确认已安装 claude 命令",
            }
            if proc_info:
            ...
            # 等待进程退出获取 exit code
            # proc.returncode is None 表示进程仍在跑（典型场景：消费者提前断开触发的
            # GeneratorExit）。此时若 persist_proc_on_consumer_exit=True（后台 Task 消费），
            # 不杀进程，保留 _active_procs 引用让它在后台跑完，后续可由 stop() 终止。
            consumer_disconnected = False
            if proc and proc.returncode is None:
                if persist_proc_on_consumer_exit:
                    consumer_disconnected = True
                    logger.info(
                        f"[claude] 消费者断开但 persist=True，保留进程 PID={proc_info.pid if proc_info else '?'} "
                        f"step={step_id}（后台继续执行）"
                    )
                else:
                    try:
                        proc.kill()
            ...
            if proc_info:
                proc_info.alive = (
                    False if not consumer_disconnected else proc_info.alive
                )
                if not consumer_disconnected:
            ...
            # 仅当进程确实结束（非 consumer 断开保留）才从 _active_procs 移除
            if not consumer_disconnected:
