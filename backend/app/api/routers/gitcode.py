...
def register_gitcode_routes(router, gitcode_service, db):
    ...
    async def get_gitcode_merge_requests(owner: str, repo: str, state: str = "all", page: int = 1, size: int = 20):
        ...
        result = gitcode_service.fetch_merge_requests(owner, repo, state, page, size)
        if result.get("error"):
    ...
    async def get_gitcode_mr_comments(owner: str, repo: str, mr_iid: int):
        ...
        if gitcode_service is None:
            raise HTTPException(status_code=503, detail="GitCode 服务未配置")
        result = gitcode_service.fetch_mr_comments(owner, repo, mr_iid)
        if result.get("error"):
            raise HTTPException(status_code=500, detail=result["error"])
        if db is not None:
            db.save_pr_comments(owner, repo, mr_iid, {**result, "platform": "gitcode"})
        return {**result, "timestamp": datetime.now().isoformat()}
    ...
    async def get_gitcode_mr_detail(owner: str, repo: str, mr_iid: int):
        ...
        if gitcode_service is None:
            raise HTTPException(status_code=503, detail="GitCode 服务未配置")
        result = gitcode_service.fetch_mr_detail(owner, repo, mr_iid)
        if result.get("error"):
            raise HTTPException(status_code=500, detail=result["error"])
        if db is not None:
            db.save_pr_detail(owner, repo, mr_iid, {**result, "platform": "gitcode"})
        return {**result, "timestamp": datetime.now().isoformat()}
    ...
    async def get_gitcode_mr_changes(owner: str, repo: str, mr_iid: int):
        ...
        if gitcode_service is None:
            raise HTTPException(status_code=503, detail="GitCode 服务未配置")
        result = gitcode_service.fetch_mr_changes(owner, repo, mr_iid)
        if result.get("error"):
            raise HTTPException(status_code=500, detail=result["error"])
        return {**result, "timestamp": datetime.now().isoformat()}

    @router.get("/gitcode/mrs/{owner}/{repo}/comments")
    async def get_all_gitcode_mr_comments(owner: str, repo: str, limit: int = 10):
        """并发获取 GitCode 所有 MR 的评论"""
        if gitcode_service is None:
            raise HTTPException(status_code=503, detail="GitCode 服务未配置")
        results = gitcode_service.fetch_all_mr_comments(owner, repo, limit)
        return {"owner": owner, "repo": repo, "results": results, "total_mrs": len(results), "timestamp": datetime.now().isoformat()}

    @router.get("/gitcode/mrs/{owner}/{repo}/details")
    async def get_all_gitcode_mr_details(owner: str, repo: str, limit: int = 10):
        """并发获取 GitCode 所有 MR 的详细信息"""
        if gitcode_service is None:
            raise HTTPException(status_code=503, detail="GitCode 服务未配置")
        results = gitcode_service.fetch_all_mr_details(owner, repo, limit)
        if db is not None:
            for item in results:
                if item.get("error") is None:
                    db.save_pr_detail(owner, repo, item["mr_iid"], {**item, "platform": "gitcode"})
        return {"owner": owner, "repo": repo, "results": results, "total_mrs": len(results), "timestamp": datetime.now().isoformat()}