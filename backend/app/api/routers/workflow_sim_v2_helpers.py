...
PLATFORM_INSTALL_PROMPT = """你正在 Docker 容器内工作，当前目录为 /workspace（宿主机 clone 下来的算子库 {repo_url}），算子名 {op_name}。
容器预装了 cmake/g++/python3/git/claude，cann 用户有 sudo NOPASSWD 权限（可 apt-get install）。
这是仿真平台在开发流程之前注入的「安装环境」步骤：**读算子库文档识别需求 → 检测现状 → 按需在容器内补装 → 产出台账**。

## 任务（严格按顺序）

### 1. 读算子库开发文档，识别开发所需环境
用 Read/Glob 阅读以下文档（找不到就跳过该文件，不要报错）：
- /workspace/README.md、/workspace/CONTRIBUTING.md
- /workspace/docs/ 下的构建说明、环境要求、开发指南
- /workspace/build.sh、/workspace/install_deps.sh（看构建/安装依赖）
- /workspace/CMakeLists.txt（看构建依赖、最低版本要求）
- /workspace/requirements.txt、/workspace/environment.md（如有）
从中提取开发本算子库需要的环境清单：
- 系统包（apt）：编译器、系统库、工具（如 build-essential、libxxx-dev）
- Python 依赖（pip）：包名 + 版本
- 工具链版本要求：cmake/g++/CANN 最低版本
- 环境变量 / 初始化脚本（source xxx.sh）

### 2. 只读检测当前环境
bash {env_setup_dir}/scripts/detect_env.sh /workspace
（对照步骤 1 的清单，标记哪些已就绪、哪些缺失）

### 3. 按需在容器内补装缺失项
对步骤 1 识别出、但步骤 2 检测缺失的，在容器内安装：
- 系统包：sudo apt-get update && sudo apt-get install -y <包名>
- Python 依赖：source /workspace/.venv/bin/activate 2>/dev/null; pip install <包名>
- CANN Toolkit：**不要自动下载安装**（几 G 大包，需配套芯片版本），缺失只在台账标❌并告知用户需预装
- 环境变量/初始化：若文档要求 source 某脚本且存在，记录下来供后续编译步骤用
安装后把实际执行的命令记入台账。

### 4. 跑配置脚本（建 venv、装 requirements.txt 等，幂等）
bash {env_setup_dir}/scripts/setup_env.sh /workspace
若生成 /workspace/.env.ascendc.sh，后续编译类步骤保持 source 它生效。

### 5. 产出 /platform-artifacts/operators/{op_name}/docs/dev-environment.md 台账
（平台产物目录 /platform-artifacts，不在算子仓 /workspace 里，不会污染代码仓、不会提交到上游）
按 {env_setup_dir}/templates/env-setup-template.md 格式，并增补一节「文档要求 vs 实际安装」：
- 文档要求的环境清单（来自步骤 1）
- 已就绪（容器预装 + 步骤 3 补装）
- 仍缺失（如 CANN，标❌）
- 实际执行的安装命令
状态行据实：✅ 就绪 / ⚠ 部分就绪 / ❌ 缺失。NPU 相关一律 ⊘ 跳过（本容器无 NPU）。

## 约束
- 先读文档再动手，安装项必须有文档依据，不要臆造。
- 只装开发所需的系统包/Python 依赖；CANN 本体不自动下载。
- 不碰 NPU（npu-smi 一律不跑）、不执行破坏性命令。
- 只做环境准备，不要开始算子开发（那是后续步骤的任务）。
- 台账必须如实：文档要求什么、装了什么、缺什么、跳过什么都写清楚。
"""
...
# ===== 工作流裁判（每个 step 完成后深度检查产出物，记录断点 + 解决方案） =====
ARBITRATOR_PROMPT = """你是一个工作流质量裁判。请检查以下步骤的执行结果并输出 JSON 报告。

🔴 只输出 JSON，不要输出任何其他文字、解释、Markdown 标记。只输出 JSON。
...
## 检查标准
1. 产出物文件是否存在且内容不为空
2. 产出物内容是否满足步骤要求的验收标准
3. 执行过程是否有明显错误（路径错误、编译失败、测试未通过、API 调用失败）
4. 是否使用了正确的 Skill（对照步骤要求的 required_skills）
5. 是否有违反约束的行为（如越权操作、跳过关键步骤）
6. 文件路径是否正确

## JSON 格式（严格）
{{"verdict": "pass" 或 "fail", "issues": [{{"problem": "问题（中文）", "severity": "CRITICAL" 或 "HIGH" 或 "MEDIUM" 或 "LOW", "category": "MISSING_FILE" 或 "CONTENT_INCOMPLETE" 或 "WRONG_PATH" 或 "SKILL_NOT_USED" 或 "ENV_MISSING" 或 "CONSTRAINT_VIOLATION" 或 "COMPILE_ERROR" 或 "OTHER", "suggestion": "修复建议（中文）", "suggestion_action": "具体修复命令"}}], "summary": "总体评价（中文）"}}

通过（无问题）示例：
{{"verdict":"pass","issues":[],"summary":"所有产出物已生成且内容达标"}}

不通过（有问题）示例：
{{"verdict":"fail","issues":[{{"problem":"文件 operators/add/docs/DESIGN.md 不存在","severity":"CRITICAL","category":"MISSING_FILE","suggestion":"创建 DESIGN.md 并填写设计内容","suggestion_action":"Write operators/add/docs/DESIGN.md"}}],"summary":"核心交付件缺失"}}
"""

PRE_PLATFORM_STEP_DEFS = [
    ("platform_install_env", "安装环境", "install_env", PLATFORM_INSTALL_PROMPT,
     ["/platform-artifacts/operators/{operator_name}/docs/dev-environment.md"]),
]
...
2. 将本次开发产出的改动纳入提交：`git -C {work_dir} add operators/ docs/` 只提交算子代码和文档产出；**.venv、.claude、.env.ascendc.sh 等平台环境产物一律不提交**（平台产物在 /platform-artifacts，不在算子仓内）；如有临时文件先 `git reset` 它们。
...
POST_PLATFORM_STEP_DEFS = [
    ("platform_upload_repo", "提交代码并推送远程", "upload_repo", PLATFORM_UPLOAD_PROMPT),
]
